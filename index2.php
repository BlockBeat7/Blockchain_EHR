...
<body>
<div class="header">
    <div class="user-info">Welcome, pichaochang</div>
...
<div class="menu" id="menu">
    <ul>
        ...
        <li onclick="changePage(1)">Recognize Idenity</li>
        <li onclick="changePage(2)">Create Health Record</li>
        <li onclick="changePage(3)">Read Health Record</li>
        <li onclick="changePage(4)">Offline Standby</li>
    </ul>
...
0x0d69b94c2bf61742d94f6a4cc4254b0037a506cf<style>
    table {
...
<div class="main" id="page-1">
    ...
    <div id="wallet-address" style="height: 0;opacity: 0"></div>
    <div class="container" id="animation-container">
        <div class="card-animation" id="card-animation">
            <img src="your-local-image-path.png" alt="Logo">
    ...
    <div class="page2-main">
        <div class="page2-left">
            ...
            <div style="display: flex;">
                <button id="save-offiline-copy" onclick="generateJSON()" style="margin-left: 0">Save offiline copy</button>
        ...
        <div class="page2-right">
            ...
            <div class="shortcut-add-box">
                ...
                <button id="add-shortcut-button" onclick="addShortcut()" disabled style="margin:0">Add</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="page3-top">
            <div class="patient-info2">
                <table>
                    ...
                    <tr>
                        <td>Address:</td>
                        <td id="patient-info-address2" colspan="0"></td>
                    </tr>
                </table>
            </div>
        </div>
...
<script>
    ...
    let registeredAddress = "0x0d69b94c2bf61742d94f6a4cc4254b0037a506cf"; // 后端返回的用户注册时的地址
    ...
    async function claimNft(ipfsUrl) {
        ...
        const res = await claimFnWeb3(contractAddress)
        if (!res) {
            return
        }
        alert("NFT claim 成功！");
    ...
    function showNewPatientForm() {
        ...
        animationContainer.style.top = '-135px';
        // 显示并渐现表单
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    data.shortcuts.forEach(shortcut => {
                        ...
                        newBox.textContent = shortcut.content;
        ...
        if (strlist.length > 1) {
            ...
            switch (strlist[0]) {
                ...
                case "NDEF_ReadInfo":
                    ...
                    const result = await sendPatientInfoRequest(); // 等待请求完成并获取返回值
                    if (result) {
    ...
    async function sendPatientInfoRequest() {
        try {
            // 模拟的 address 参数
            const address = '0x0f639d2f7ecd88d1bddfaa8fa5e2c5bb6d7c7bcf';
            ...
            if (data.status === 'success') {
                ...
                $.ajax({
                    type: "GET",
                    url: `http://114.132.75.165/api/api/claim?address=${data.patient.address}`,
...
<script>
    ...
    function changePage(page){
        if(page == 1){
            ...
            //读取缓存里的patient_info,如果没有则alert提示，不进行下面的处理，如果有的话，把patient_info的数据插入到patient-info-name和patient-info-address中
            var patient_info = JSON.parse(localStorage.getItem("patient_info"));
            ...
            //读取缓存里的patient_info,如果没有则alert提示，不进行下面的处理
            var patient_info = JSON.parse(localStorage.getItem("patient_info"));
            if(!patient_info.name){
                alert("请先完成患者信息录入");
                return;
...
// 发起请求以获取 NFT 数据
        $.ajax({
            ...
            success: function(data) {
                ...
                const healthTableBody = $('#health-record');
                ...
                if (assets && assets.length > 0) {
                    assets.forEach(function(asset) {
                        const name = asset.title || "Unknown NFT";
                        const description = asset.description || "No description available";
                        const link = asset.external_link || `https://opensea.io/assets/${chain}/${asset.token_id}`;
